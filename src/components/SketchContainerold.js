    ...
    for (let _x = 0, i = 0; _x < this.x_dim; _x++) {
      for (let _y = 0; _y < this.y_dim; _y++, i++) {
        ...
        this.stroke_color != undefined
          ? p5.stroke(this.stroke_color)
          : p5.noStroke();
        this.fill_color != undefined
          ? p5.fill(this.fill_color, this.cells[i] > 0.5 ? 255 : 0)
          : p5.noFill();

        p5.rect(
          p5.floor(pos_x),
          p5.floor(pos_y),
          p5.ceil(this.width / this.x_dim),
          p5.ceil(this.height / this.y_dim)
        );
...
export default class SketchContainer extends React.Component {
  ...
  render() {
    return <div id="canvascontainer" ref={this.ref}></div>;
  }
}
